...
df <-  df %>% 
  mutate(pred_letramento = round(predict(rpart_letramento, df)),
         pilar_letramento = case_when(
           pred_letramento >= 288 ~ 'A',
           pred_letramento >= 279 ~ 'B',
           pred_letramento >= 268 ~ 'C',
           pred_letramento >= 267 ~ 'D',
           pred_letramento >= 261 ~ 'E',
           pred_letramento >= 254 ~ 'F',
         ))
...
df <-  df %>% 
  mutate(pred_localizacao = round(predict(rpart_localizacao, df)),
         pilar_localizacao = case_when(
           pred_localizacao == 289 ~ 'A',
           pred_localizacao == 276 ~ 'B',
           pred_localizacao == 266 ~ 'C',
           pred_localizacao == 267 ~ 'D',
           pred_localizacao == 257 ~ 'E',
           pred_localizacao == 253 ~ 'E',
         ))
